...
// [4차 과제] 배열과 포인터를 매개변수로 받는 함수 선언
// 포인터를 이용해 다중 반환(Call-by-Reference) 효과를 냅니다.
void analyze_weekly_weights(float *weight, int size, float *avg, float *max, float *min);
...
int main()
{
  ...
  // [4차 과제] 7일간의 체중을 저장할 배열 선언
  float weekly_weights[7];
  float avg_weight, max_weight, min_weight;   // 포인터로 값을 받아올 지역 변수
  ...
  while(1)
  {
    ...
    printf("2. 주간 체중 기록 및 분석\n");
    printf("3. 누적 이용 횟수 조회\n");
    printf("4. 프로그램 종료\n");
    ...
    else if(choice == 2)
    {
      ...
      for(i=0; i<7; i++)
      {
        printf("%d일차 체중(kg): ", i + 1);
        scanf("%f", &weekly_weights[i]);
      }

      analyze_weekly_weights(weekly_weights, 7, &avg_weight, &max_weight, &min_weight);
...
// [4차 과제] 배열과 포인터를 활용한 분석 함수
// 배열(weight)과 값을 저장할 변수들의 메모리 주소(avg, max, min)를 받습니다.
void analyze_weekly_weights(float *weights, int size, float *avg, float *max, float *min)
{
  float sum = 0.0;
  *max = weights[0];
  *min = weights[0];
